  ...
  schema.pre('save', function (next) {
    ...
    var parse = previous => {
      paths.forEach(path => {
        ...
        var type = typeof path !== 'string' ? path.type.toString().toLowerCase() : 'string';

        if (typeof raw === 'object' && type === 'stringarray') {

          updateValue = [];

          raw.forEach(value => {
            
            var objValue = titlize(value);
            
            if (path.trim !== false && options.trim !== false) {
              objValue = objValue.trim();
            }

            updateValue.push(objValue);
